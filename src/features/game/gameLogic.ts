import { IStreak, IStreakStats, IBoardLayout, PlayerId, IPosition, IMoveSpecs } from './game.types';
...
export function updateMoves(moves: IMove[], playerId: PlayerId, moveSpecs: IMoveSpecs, gameOver: boolean, boardLayout: IBoardLayout): IMove[] {
    return [
        ...moves,
        {
            playerId,
...
export function evaluateBoard(
    ...
    for (let row = 0; row < boardlayout.length; row++) {
        for (let col = 0; col < boardlayout[row].length; col++) {
            ...
            const horizontal = streakRange.map((pos) => {
                try {
            ...
            const vertical = streakRange.map((pos) => {
                try {
            ...
            const rightDiag = streakRange.map((pos) => {
                try {
            ...
            const leftDiag = streakRange.map((pos) => {
                try {
